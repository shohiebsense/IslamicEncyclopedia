...
class UlamaTabView extends StatefulWidget {
  //final bool isScrolling;
  final PageStorageKey key;
  final BiographyDao biographyDao;

  UlamaTabView(this.key, this.biographyDao) : super(key: key);

  @override
  UlamaTabViewState createState() => UlamaTabViewState();
}

class UlamaTabViewState extends State<UlamaTabView>
    with AfterLayoutMixin<UlamaTabView> {
  ...
  Future<List<Biography>> getList() async {
    return getDao().then((value) {
      return value.findAllUlama().then((value) {
        biographyList = value;
                                      ...
                                      child: ClipRRect(
                                        ...
                                        child: Container(
                                          ...
                                          child: Align(
                                            ...
                                            child: Text((index + 1)
                                                .toString()
                                                .toPersianDigit(), style: TextStyle(color: Colors.white, fontSize: 18),),
                                          ),
                                        ),
                                      )),
                                ),
